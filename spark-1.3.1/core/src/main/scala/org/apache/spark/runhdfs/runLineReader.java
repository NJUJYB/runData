...
public class runLineReader implements Closeable {
  ...
  public String fileName, fileOrigPath = null;
  public ArrayList<String> expLogs = new ArrayList<String>();
  public ArrayList<Long> times = new ArrayList<Long>(0);
  public byte[] splitBuf = null;
   ...
   * @throws IOException
   */
  public runLineReader(InputStream in) {
   ...
   * @throws IOException
   */
  public runLineReader(InputStream in, int bufferSize) {
	this.in = in;
   ...
   * @throws IOException
   */
  public runLineReader(InputStream in, Configuration conf) throws IOException {
   ...
   */
  public runLineReader(InputStream in, byte[] recordDelimiterBytes) {
	this.in = in;
   ...
   * @throws IOException
   */
  public runLineReader(InputStream in, Configuration conf,
  ...
  // runData
  public runLineReader(InputStream in, Configuration conf, byte[] recordDelimiterBytes,
	Path filePath, long start, long end) throws IOException {
	this(in, conf, recordDelimiterBytes);
	bytes = end - start;
	fileName = filePath.getName();
	fileLock = new File("/home/jyb/Desktop/hadoop/hadoop-2.2.0/logs/locks/" + fileName);
	if(fileLock.exists()) {
	  fileOrigPath = filePath.toString();
	  // fileLock.delete(); // Only use for tests
	}
	recordLogs("StartInit");
	recordLogs(filePath +" " + fileName + " " + bytes);
